...
export function normalize(input: any[]): Row[] {
  const rows: Row[] = input.map((r) => {
    ...
    const contracts = toNum(r["청약"]) ?? toNum(r["contracts"]) ?? toNum(r["청약건수"]) ?? 0;

    const date = r["일자"] || r["date"] || undefined;

    return {
      date,
      month: (month || inferMonth(date) || "").toString(),
      teamMember: (team || "").toString(),
      productGroup: (pg || "").toString(),
      premiumType: (String(ptRaw).includes("계속") ? "계속" : "초회"),
      premium: premium,
      meetings, proposals, contracts
    };
  }).filter(r => r.month);
